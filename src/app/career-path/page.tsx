    ...
    return (
        <div className="min-h-screen bg-[#0a0f1e] text-white font-sans selection:bg-cyan-500/30">
            ...
            <main className="container-custom mx-auto py-12 px-4">
                ...
                {step === 1 && (
                    <div className="max-w-3xl mx-auto bg-slate-900/50 border border-slate-800 rounded-3xl p-8 backdrop-blur-xl relative overflow-hidden">

                        {/* Scanning Effect Overlay */}
                        {isAnalyzing && (
                            <div className="absolute inset-0 z-50 bg-black/80 flex flex-col items-center justify-center backdrop-blur-sm">
                                <Loader2 className="w-16 h-16 text-cyan-400 animate-spin mb-4" />
                                <div className="text-cyan-300 font-mono text-lg animate-pulse">ANALYZING DOCUMENTS...</div>
                                <div className="text-slate-500 text-sm mt-2">Extracting grades, streams, and certifications</div>
                            </div>
                        )}

                        <div className="border-2 border-dashed border-slate-700 rounded-2xl p-12 text-center hover:border-cyan-500/50 hover:bg-cyan-500/5 transition group cursor-pointer relative bg-slate-950/50">
                            <input
                                type="file"
                                multiple
                            ...
                            />
                            <div className="w-20 h-20 bg-slate-900 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition border border-slate-800 shadow-[0_0_30px_rgba(0,255,255,0.1)]">
                                <Upload className="w-10 h-10 text-cyan-400" />
                            </div>
                            <h3 className="text-2xl font-bold text-white mb-2">Drop Academic Documents</h3>
                            <p className="text-slate-400">10th, 12th, Bachelor's, Transcripts, Certificates</p>
                        </div>

                        {documents.length > 0 && (
                            <div className="mt-8 space-y-3">
                                <h4 className="text-sm font-bold text-slate-400 uppercase tracking-wider">Queue ({documents.length})</h4>
                ...
                {step === 2 && intelligence && profile && (
                    <div className="animate-fade-in-up space-y-8">
                        ...
                        {/* 1. AI Insight Panel */}
                        <div className="bg-gradient-to-r from-purple-900/40 to-blue-900/40 border border-purple-500/30 p-8 rounded-3xl relative overflow-hidden">
                            ...
                            <div className="flex items-start gap-4">
                                ...
                                <div>
                                    <h2 className="text-xl font-bold text-white mb-2">AI Strategic Assessment</h2>
                                    <p className="text-purple-100/80 leading-relaxed text-lg">{intelligence.aiInsight}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            <div className="lg:col-span-1 space-y-6">
                                <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-2xl relative">
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            ...
                                            <div className="text-3xl font-bold text-white">{profile.gpa} <span className="text-sm text-slate-400 font-normal">/ 10.0</span></div>
                                        </div>
                                        <div>
                                            <div className="text-sm text-slate-500">Dominant Stream</div>
                                            <div className="text-xl font-bold text-cyan-400">{intelligence.academicSummary.dominantStream}</div>
                                        </div>
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {profile.subjectStrength.map((sub, i) => (
                                                    <span key={i} className="px-2 py-1 bg-slate-800 text-slate-300 text-xs rounded border border-slate-700">
                                                        {sub.subject} <span className="text-green-400">{sub.score}%</span>
                                                    </span>
                                                ))}
                                            </div>
                                        </div>
                                ...
                                <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-2xl relative">
                                    <div className="flex justify-between items-center mb-4">
                                        ...
                                        <span className={`px-2 py-0.5 rounded text-xs font-bold ${intelligence.riskAnalysis.level === 'Low' ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
                            ...
                            <div className="lg:col-span-2 space-y-6">
                                <h3 className="text-xl font-bold flex items-center gap-2">
                                    <Globe className="w-5 h-5 text-blue-400" /> Global Opportunities
                                </h3>

                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {intelligence.globalPaths.map((path, i) => (
                                        <div key={i} className="bg-slate-900/40 border border-slate-800 p-6 rounded-2xl hover:border-cyan-500/30 transition group relative overflow-hidden">
                                            <div className="absolute top-0 right-0 p-4 opacity-50">
                                                <TrendingUp className="w-12 h-12 text-slate-800 group-hover:text-cyan-900/50 transition" />
                                            </div>

                                            <div className="relative z-10">
                                                <div className="flex justify-between items-start mb-2">
                                                    <div className="text-xs font-bold text-slate-500 uppercase tracking-wider">{path.country}</div>
                                                    <div className="flex items-center gap-1 text-green-400 text-xs font-bold bg-green-900/20 px-2 py-1 rounded">
                                                        ROI: {path.roi}
                                                    </div>
                                                </div>
                                                <h4 className="text-lg font-bold text-white mb-1 group-hover:text-cyan-400 transition">{path.title}</h4>

                                                <div className="grid grid-cols-2 gap-4 mt-4">
                                                    <div>
                                                        <div className="text-[10px] text-slate-500 uppercase">Cost</div>
                                                        <div className="text-sm font-medium text-slate-200">{path.cost}</div>
                                                    </div>
                                                    <div>
                                                        <div className="text-[10px] text-slate-500 uppercase">Duration</div>
                                                        <div className="text-sm font-medium text-slate-200">{path.duration}</div>
                                                    </div>
                                                    <div>
                                                        <div className="text-[10px] text-slate-500 uppercase">Visa Chance</div>
                                                        <div className="text-sm font-bold text-blue-400">{intelligence.visaConfidence[path.country] || 80}%</div>
                                                    </div>
                                                    <div>
                                                        <div className="text-[10px] text-slate-500 uppercase">Avg Salary</div>
                                                        <div className="text-sm font-bold text-green-400">{intelligence.roiForecast[path.country] || "$??"}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <h3 className="text-xl font-bold flex items-center gap-2 mt-8">
                                    <Briefcase className="w-5 h-5 text-orange-400" /> Professional Trajectories
                                </h3>

                                <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
                                    <div className="flex flex-wrap gap-3">
                                        {intelligence.careerRoles.map((role, i) => (
                                            <div key={i} className="px-4 py-2 bg-slate-800 rounded-lg border border-slate-700 hover:border-orange-500/50 transition cursor-default text-sm text-slate-200">
                                                {role}
                                            </div>
                                        ))}
                                    </div>
                                ...
                                <div className="flex justify-end gap-4 mt-8">
                                    <button onClick={() => setStep(1)} className="px-6 py-2 rounded-xl text-sm font-bold text-slate-400 hover:text-white transition">
                                        Upload More Documents
                                    </button>
                                    <button className="px-6 py-2 rounded-xl text-sm font-bold bg-white text-black hover:bg-cyan-400 hover:text-black transition shadow-lg shadow-cyan-500/20">
